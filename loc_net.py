...
def load_images(frames_directory, num_images_to_load, first_image_index, step):
	...
	mat = scipy.io.loadmat(frames_directory + 'centerline.mat')
		...
		bin_array_flattened = np.reshape(bin_array, (1, num_x_bins*num_y_bins)) / 100.
...
if(mode == LOAD_MODEL):
	...
	for k in range(num_test_sample_images):
		...
		#generate net's prediction and the 'true' answer computed from centerline
		true_ans = y_test[test_image].reshape(num_x_bins, num_y_bins)
